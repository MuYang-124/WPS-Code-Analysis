...
def parse_html():
    ...
    try:
        ...
        code_block = soup.find(
            'code', class_="code-block-content code-block-theme-style")

        if code_block:
            inner_html = str(code_block)
            parsed_code = extract_code_from_html(inner_html)
            ...
            status_bar.config(text="未找到目标<code>标签", foreground="orange")
    except Exception as e:
